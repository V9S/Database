...
SELECT * FROM GAMS_JC_ACCOUNTSYSTEM;

SELECT * FROM GAMS_JC_GAINMANNER ,GAMS_JC_VALUETYPE,GAMS_JC_ACCOUNTSYSTEM gja ;


SELECT * FROM GAMS_JC_GAINMANNER LEFT JOIN GAMS_JC_VALUETYPE ON 1=1;

SELECT count(gjp.CODE),gjp.CODE FROM GAMS_CARD gc LEFT JOIN GAMS_JC_PERSONNEL gjp ON gc.SHIYR = gjp.id GROUP BY gjp.CODE ORDER BY count(gjp.CODE) DESC ;

SELECT gc.billtime FROM GAMS_CARD gc ;