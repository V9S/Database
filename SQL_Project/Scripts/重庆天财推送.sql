...
SELECT FIELDMAPPING FROM gams_jc_cwparam;
...
select * from gams_jc_cwparam WHERE title = '增减值变动推送(天财)';
...
SELECT DANJ_NEW FROM GAMS_ASSETCHANGE_DETAIL gad WHERE gad.MASTER_ID = (SELECT id FROM GAMS_ASSETCHANGE ga WHERE ga.BILL_CODE = 'ZYBD2020000028');
