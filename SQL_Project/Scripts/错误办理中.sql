...
BEGIN
	...
	tt.id AS id,
	tt.state AS state,
	tt.workflowProcessInstance AS workflowProcessInstance,
...
FROM
	(
	SELECT
		gwbp.id AS id, '已驳回' AS state, gwbp.workflowProcessInstance AS workflowProcessInstance, gwbp.businessObjectName AS businessTypeName, gwbp.businesscode AS billcode, gwbp.submitTime AS submitTime, gwbp.businessDataId AS businessDataId, gwbp.businessTypeName AS businessType, gwbp.submitterId AS identityId, identity.name AS identityName
	FROM
...
UNION ALL
	SELECT
		gwbp.id AS id, '已驳回' AS state, gwbp.workflowProcessInstance AS workflowProcessInstance, gwbp.businessObjectName AS businessTypeName, gwbp.businesscode AS billcode, gwbp.submitTime AS submitTime, props.master AS businessDataId, gwbp.businessTypeName AS businessType, gwbp.submitterId AS identityId, identity.name AS identityName
	FROM
		BusinessProcessInstance AS gwbp
	LEFT JOIN IDENTITY AS IDENTITY ON
		gwbp.submitterId = identity.id
